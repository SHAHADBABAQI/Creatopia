...
struct Shake: View {
    ...
    private let footerSize: CGFloat = 28
    @State private var didShake = false
    @State private var navigateToAfterShake = false  // ✅ Add navigation trigger
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                NavigationLink(
                    destination: AfterShake(),
                    isActive: $navigateToAfterShake
                ) {
                    EmptyView()
                }
                .hidden()
            ...
            .modifier(ShakeDetector {
                withAnimation(.spring(response: 0.45, dampingFraction: 0.75)) {
                    didShake = true
                    // ✅ Navigate immediately after shake
